...
const columns: ColumnDef<TxLogCompanyDto>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return <div className="text-center font-medium">{row.getValue("washing")}</div>
    },
  },
  {
  ...
  const handleRefresh = async () => {
    const fetchData = async () => {
      try {
        ...
        const httpResp = await fetch(`${GetHostLocation()}/api/v1/tx-log/companies?from=${from}&to=${to}`)
