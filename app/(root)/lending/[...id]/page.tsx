...
import { ResponseBody, TxLogDepartmentDto, TxLogDetailDto, TxLogTracking } from "@/dto/response"
...
export default function LendingTagScreen({ params }: { params: Promise<{ id: string }> }) {
   ...
   const [txLog, setTxLog] = React.useState<TxLogDepartmentDto>()
   ...
   React.useEffect(() => {
      const fetchData = async () => {
         try {
            const httpResp = await fetch(`${GetHostLocation()}/api/v1/tx-log/departments/${id}`)
            const jsonResp: ResponseBody<TxLogDepartmentDto> = await httpResp.json()
            if (jsonResp.success) {
   ...
   return (
      <div className="w-full px-2 sm:px-6">
         {txLog && (
            <div>
               <h2 className="text-3xl font-bold mb-4 text-green-600">Lịch sử Khoa Mượn / Trả Đồ</h2>
               <div className="flex flex-1 justify-between p-4 mb-4 rounded-md border">
                  <div className="flex items-center">
                     ...
                     <p className="text-xl font-bold text-blue-600">{txLog.num_lending + txLog.num_returned}</p>
                  </div>
                  <div className="flex items-center">
                  ...
                  <div className="flex items-center">
                     <p className="font-semibold text-gray-600 mr-2">Chưa trả</p>
                     <p className="text-xl font-bold text-red-600">{txLog.num_lending}</p>
                  </div>
               </div>
            </div>
         )}
